...
# skeleton code in the file hash_map_oa.py.
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        # If the bucket does not contain a key and value,
        # then simply insert the key-value pair and increment size
        if self._buckets.get_at_index(h_key) is None:
            self._buckets.set_at_index(h_key, HashEntry(key, value))
            self._size += 1

        else:
            c = 1
            q_key = h_key
            while self._buckets.get_at_index(q_key):

                # If the key is found in the hash map, replace the value
                if self._buckets.get_at_index(q_key).key == key:
                    if self._buckets.get_at_index(q_key).is_tombstone:

                        # If the previous entry was a tombstone,
                        # update the value and increment size
                        self._buckets.set_at_index(q_key,
                                                   HashEntry(key, value))
                        self._size += 1
                        self._buckets.get_at_index(q_key).is_tombstone = False
                    else:
                ...
                q_key = (h_key + (c ** 2)) % self._capacity
                c += 1

            # If the bucket is empty or no key matches,
            # add a new key-value pair and increment size
            self._buckets.set_at_index(q_key, HashEntry(key, value))
            self._size += 1
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        must remain in the new hash map,
        ...
        # Adjust the new capacity to the next prime number
        ...
        # Special case for new capacity of 2 (since 2 is prime)
        if new_capacity == 2:
        ...
        # Update the buckets with the new hash map's buckets, the new size
        # the new capacity
    ...
    def remove(self, key: str) -> None:
        """
        This method removes the given key and its
        associated value from the hash map.
        """
        for entry in self:
            if entry:
                if entry.key == key and not entry.is_tombstone:
    ...
    def get_keys_and_values(self) -> DynamicArray:
        """
        This method returns a dynamic array where each
        index contains a tuple of a key/value pair
    ...
    def __next__(self):
        """
        This method will return the next entry in the hash map,
...
if __name__ == "__main__":
    ...
    print(m)
    for entry in m:
        print('K:', entry.key, 'V:', entry.value)
